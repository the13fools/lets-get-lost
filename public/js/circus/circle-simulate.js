circleSim = (function () {
	...
	exp.simulate = function (time) {
		...
		for (i = 0; i < 2; i++) {
			...
			// driving logic
			if (exp.yFreq != 0) {
		    	system.particles[0].position.setY(Math.sin(step / 50 * exp.yFreq) / 5 + 1);
			    system.particles[0].position.setX(0);
			}
			system.fixedPoints[0].position.setX(Math.sin(step / 50 * exp.xFreq) / 10);
	//		system.fixedPoints[0].position.setY(0);
		}
