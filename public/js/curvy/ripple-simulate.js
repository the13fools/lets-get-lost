rippleSim = (function () {
	...
	exp.system = rippleInit.system;
	exp.rippleGeometry = rippleInit.rippleGeometry;
	...
	exp.render = function() {
		...
		var ripple = exp.system.particles;


		var il = ripple.length;
		for ( var i = 0; i < il + 1; i ++ ) {
			exp.rippleGeometry.vertices[ i + 1 ]
				.copy( ripple[ i % il ].position )
				.multiplyScalar(350);

		}

		exp.rippleGeometry.computeFaceNormals();
		exp.rippleGeometry.computeVertexNormals();

		exp.rippleGeometry.normalsNeedUpdate = true;
		exp.rippleGeometry.verticesNeedUpdate = true;

		if (rotateCamera) { 
			rippleThree.camera.position.x = Math.cos( timer / 500 ) * 1500;
			rippleThree.camera.position.z = Math.sin( timer / 500 ) * 1500;
			timer ++;
		}

		rippleThree.camera.lookAt( rippleThree.scene.position );

		rippleThree.renderer.render( rippleThree.scene, rippleThree.camera );
